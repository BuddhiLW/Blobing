...
**[bb-mcp](https://github.com/hive-agi/bb-mcp)** solves this: a Babashka wrapper (~50MB) that connects multiple lings to a single shared hive-mcp JVM via nREPL.
...
The system is ~44k lines of code (235 commits over 28 days, largely self-hosted):
...
*Methodology: Self-measured across 47 development sessions over 8 weeks, single developer, Alienware M16 R2 (Intel Core Ultra 9 185H, 30GB RAM, Ubuntu 24.04). Tasks ranged from bug fixes (~30min solo equivalent) to multi-file refactors (~3hrs solo equivalent) to new feature implementations. "Solo Claude" baseline = Claude Code CLI with manual context management. Caveat: dogfooding bias—we built the tool, so treat multipliers as directional, not precise.*
...
**Repository**: [github.com/hive-agi/hive-mcp](https://github.com/hive-agi/hive-mcp)
...
- [bb-mcp](https://github.com/hive-agi/bb-mcp): 800-line Babashka thin-client showing the MCP↔JVM bridge pattern
